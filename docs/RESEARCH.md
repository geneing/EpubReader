...
- Readium 3.4.0's TTS Media3 session adapter contains an `AudioFocusManager` and requests focus when its player starts. Pixel logs also showed a separate service-side focus requester competing with Readium; the service requester was removed so Readium is the single focus owner. Re-test transient/permanent focus loss and opt-in long-interruption resume on device. See [Android audio focus guidance](https://developer.android.com/media/optimize/audio-focus).
...
- The app uses the pinned `2026.09` / model API 1 manifest for download and includes its manifest locally; model-release endpoint availability and a fresh-install download are not yet verified. Host-to-Pixel ADB provisioning for 15 required model files was verified with pinned per-file hashes on 2026-09-24.
...
3. Both engines flow through the same Readium TTS navigator/location state; Pocket is selected through an app-owned `TtsEngineProvider` that explicitly binds to the bundled Android service. Pixel 10 confirmed service discovery and engine startup, but audible synthesis/performance checks remain.
