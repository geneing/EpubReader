...
- The app is currently a single `:app` Compose module. Its version catalog pins Kotlin 2.4.20, AGP 9.4.0, Gradle 9.7.1, Compose BOM 2026.09.00, Readium 3.4.0, and PDFium adapter 3.4.0. `minSdk`/`targetSdk` are API 36; Compose currently requires `compileSdk` 37.
...
- Foreground EPUB narration uses Readium 3.4.0's Android TTS navigator. Selection context actions start narration at the selected locator, and utterance locators drive visual following/highlighting. Speech-rate preference is applied. PDF speech, per-book speech preferences, background media-session controls, exact locator persistence, bookmarks, and Pocket TTS remain unimplemented. Pocket is shown as unavailable in Settings until its engine is integrated.
...
- Preferred first integration: include/adapt the core and service modules as source and register Pocket as an Android TTS engine. `TextToSpeech` can then speak through the system engine interface, allowing Readium's Android TTS adapter to drive both providers with the same utterance lifecycle. Validate how engine package selection and service discovery are surfaced by the Readium API.
...
- The upstream library currently declares minSdk 31. The product's API 36 minimum is compatible. Confirm the actual manifest/service declaration, API behavior, and release model URLs for the exact pinned revision.
...
- Model delivery uses the upstream `ModelSource` / `ReleaseModelSource` and SHA-256 manifest where possible. Model hosting/release availability must be verified before shipping. Provide resumable download and enough free-space checks; report model version and checksum state.
...
- **Development/test provisioning:** Generate/download the exact pinned model pack on the Windows host (using WSL only if upstream conversion scripts require POSIX), then upload it to the connected Pixel 10 with `adb push` into the app-specific Pocket model directory recognized by the pinned `PocketTtsModels` source. Implement a debug provisioning task/script that validates filenames and SHA-256 hashes and reports the resolved destination.
...
- **Pocket integration boundary:** Source-including two modules is likely the shortest route. Assess a published Maven artifact or standalone included build only if upstream starts publishing a stable library.
