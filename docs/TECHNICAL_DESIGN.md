...
- Pocket source is pinned to `geneing/PocketTTS-LiteRT` commit `0354739b7af355da804fd676b69ac03a67aa3178`; only its MIT-licensed core/service source is vendored under `third_party/`. Model manifest/API version is `2026.09`/`1`; model weights and voice files are downloaded or ADB-provisioned into app-specific storage and are not committed or bundled in the APK. See `third_party/PocketTTS-LiteRT/UPSTREAM.md` for provenance and `scripts/provision-pocket-models.ps1` for verified Pixel provisioning.
