...
- **Active branch:** `main` (Pocket TTS checkpoint merged from `feature/pocket-tts`).
- **Current milestone:** Complete Pocket TTS end-to-end verification on Pixel 10 while stabilizing audio-focus/interruption behavior and preserving Bluetooth resume policy.
...
5. Confirm audible Pocket TTS narration from the EPUB on Pixel 10; test voice selection, pause/stop/cancellation, offline behavior, first-audio latency, and memory use.
6. Recheck audio focus with another media app and exercise Bluetooth disconnect/reconnect, notification, lock-screen, and car controls on available routes.
7. Persist exact Readium locator JSON and validate process recreation.
